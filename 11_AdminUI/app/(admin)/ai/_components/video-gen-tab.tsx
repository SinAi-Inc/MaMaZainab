...
import { Copy, Check, Video, ExternalLink, Users, RotateCcw, Sparkles, Loader2, AlertTriangle, X } from "lucide-react";
import { toast } from "sonner";
import { Card, CardBody } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/input";
import { recordGeneration } from "@/lib/generations/actions";
import { submitVideoJob, pollVideoJob, cancelVideoJob } from "@/lib/video/actions";
import { estimateVideoCost, formatUsd } from "@/lib/video/cost";
import type { VideoJob, VideoTier } from "@/lib/video/schema";
...
export function VideoGenTab({ characters, providers }: { characters: Character[]; providers: import("@/lib/video/provider-info").ProviderSummary[] }) {
  ...
  const [providerId, setProviderId] = useState<string>(() => {
    return configured.find((p) => p.tier === "hero")?.id ?? configured[0]?.id ?? "";
  });
  const [activeJob, setActiveJob] = useState<VideoJob | null>(null);
  const [violations, setViolations] = useState<{ type: string; message: string }[]>([]);
  ...
  // ---------- Inline generation ----------
  const selectedProvider = providers.find((p) => p.id === providerId) ?? configured[0];
  const costEstimate = useMemo(
    () => (selectedProvider ? estimateVideoCost(selectedProvider.id, duration) : null),
    [selectedProvider, duration],
  );
  const isTerminal = activeJob?.status === "completed" || activeJob?.status === "failed" || activeJob?.status === "canceled";

  // Poll every 3s while job is running
  useEffect(() => {
    if (!activeJob || isTerminal) {
      if (pollRef.current) window.clearInterval(pollRef.current);
      pollRef.current = null;
      return;
    }
    pollRef.current = window.setInterval(async () => {
      const updated = await pollVideoJob(activeJob.id);
      if (updated) {
        setActiveJob(updated);
        if (updated.status === "completed") toast.success("Video ready");
        if (updated.status === "failed") toast.error(`Video failed: ${updated.error || "unknown"}`);
      }
    }, 3000);
    return () => {
      if (pollRef.current) window.clearInterval(pollRef.current);
  ...
  async function handleGenerate() {
    const full = buildFullPrompt();
    if (!full.trim() || full.length < 20) { toast.error("Write a longer prompt first"); return; }
    if (!selectedProvider) { toast.error("No video provider configured"); return; }
    setSubmitting(true);
    setViolations([]);
    try {
      const result = await submitVideoJob({
        ...
        tier,
        prompt: full,
        characterAnchors: anchorValues,
        aspectRatio: aspect,
        durationSec: duration,
      });
      if (!result.ok) {
        toast.error(result.error);
        if (result.violations) setViolations(result.violations);
        return;
      }
      setActiveJob(result.job);
      toast.success(`Submitted to ${selectedProvider.label}`);
    } catch (e) {
  ...
  async function handleCancel() {
    ...
    setActiveJob({ ...activeJob, status: "canceled" });
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {anchor.modeLabel ? (
                      <span className={`inline-block mt-0.5 text-[9px] font-semibold uppercase tracking-wide rounded-full px-1.5 py-0.5 ${
                        checked
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardBody className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {configured.length === 0 && (
                <span className="flex items-center gap-1 text-[10px] text-amber-700">
                  <AlertTriangle className="size-3" /> No provider configured
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {(["hero", "draft"] as VideoTier[]).map((t) => (
                <button
                  key={t}
                  onClick={() => {
                    setTier(t);
                    const next = configured.find((p) => p.tier === t);
                    if (next) setProviderId(next.id);
                  }}
            ...
            <div>
              ...
              <select
                ...
                className="w-full text-xs border border-border-strong rounded-md px-2 py-1.5 bg-white"
              >
                {configured.map((p) => (
              ...
              {selectedProvider && (
                <p className="text-[10px] text-muted mt-1">
                  max {selectedProvider.maxDurationSec}s · {selectedProvider.aspects.join(", ")}
                  {selectedProvider.characterRefs && " · char refs ✓"}
                </p>
              )}
            </div>

            {/* Cost estimate */}
            {costEstimate && selectedProvider?.configured && (
              <div className="flex items-center justify-between text-xs bg-surface rounded-md px-2.5 py-2 border border-border">
                <span className="text-muted">Est. cost</span>
                <span className="font-mono font-semibold text-brand-green-deep">
                  {formatUsd(costEstimate.estimatedUsd)}
                </span>
              </div>
            )}

            {/* Brand-Lock violations */}
            {violations.length > 0 && (
              <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 space-y-1">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold text-red-800">
                  <AlertTriangle className="size-3" /> Brand-Lock blocked submission
                </div>
                {violations.map((v, i) => (
                  <p key={i} className="text-[10px] text-red-700">· {v.message}</p>
                ))}
              </div>
            )}
            ...
            <Button
              ...
              disabled={submitting || !selectedProvider?.configured || (!!activeJob && !isTerminal)}
              className="w-full"
            >
              {submitting ? <><Loader2 className="size-4 animate-spin" /> Submitting...</> : <><Sparkles className="size-4" /> Generate Video</>}
            </Button>
              ...
              <div className="rounded-md border border-border bg-surface px-3 py-2 space-y-2">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="text-muted">
                    Status: <span className={`font-semibold ${
                      activeJob.status === "completed" ? "text-emerald-700" :
                      activeJob.status === "failed" ? "text-red-700" :
                      activeJob.status === "canceled" ? "text-muted" :
                      "text-blue-700"
                    }`}>{activeJob.status}</span>
                  </span>
                  {!isTerminal && (
                    <button onClick={handleCancel} className="text-[10px] text-red-600 hover:underline flex items-center gap-0.5">
                ...
                {!isTerminal && (
                  <div className="flex items-center gap-2 text-[11px] text-muted">
                    ...
                    Polling provider every 3s…
                  </div>
                )}
                {activeJob.status === "completed" && activeJob.outputUrl && (
                  <video
                    src={activeJob.outputUrl}
                    poster={activeJob.posterUrl || undefined}
                    controls
                    className="w-full rounded-md border border-border bg-black"
                  />
                )}
                {activeJob.status === "failed" && (
                  <p className="text-[11px] text-red-700">{activeJob.error || "Unknown error"}</p>
                )}
              </div>
            )}
          </CardBody>
        ...
        {/* Service launchers */}
        <div>
          <p className="text-xs font-medium text-muted uppercase tracking-wider mb-2">Or launch in external service</p>
